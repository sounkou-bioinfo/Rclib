#' Strip ANSI escape codes from a character vector
#' @param x Character vector possibly containing ANSI escape codes
#' @return Character vector with ANSI codes removed
#' @export
strip_ansi <- function(x) {
    gsub("\\x1b\\[[0-9;]*[mK]", "", x, perl = TRUE)
}
clib_commands <- c(
    "clib",
    "clib-install",
...
#' clib_bin_install
#' Install the clib binaries to a specified directory
#' this is usefull to get clib into your PATH
#' @param dest_dir Destination directory to install the binaries
#' @return NULL
#' @export
